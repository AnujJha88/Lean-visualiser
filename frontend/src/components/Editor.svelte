<script lang="ts">
  ...
  function registerLeanLanguage() {
    ...
    monaco.languages.setMonarchTokensProvider('lean4', {
      keywords: [
        'theorem', 'lemma', 'def', 'example', 'axiom', 'constant',
        'structure', 'class', 'instance', 'inductive', 'where',
        'if', 'then', 'else', 'match', 'with', 'do', 'return',
        'let', 'have', 'show', 'by', 'fun', 'forall', 'exists',
        'import', 'open', 'namespace', 'section', 'variable',
        'private', 'protected', 'public', 'partial', 'unsafe',
        'set_option', 'attribute', 'deriving',
      ],
      tactics: [
        'intro', 'intros', 'exact', 'apply', 'rw', 'rewrite', 'simp',
        'constructor', 'cases', 'induction', 'have', 'let', 'show',
        'assumption', 'contradiction', 'trivial', 'rfl', 'ring',
        'linarith', 'omega', 'decide', 'norm_num', 'ext', 'funext',
        'congr', 'left', 'right', 'use', 'exists', 'obtain', 'rcases',
      ],
      types: ['Prop', 'Type', 'Sort', 'Nat', 'Int', 'Bool', 'String', 'List', 'Option'],
      operators: ['→', '←', '↔', '∧', '∨', '¬', '∀', '∃', '⊢', '·', ':=', '|'],
